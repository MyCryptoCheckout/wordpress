...
class Etherscan extends HttpProvider implements \phpEther\Web3\Providers\Provider {
	...
	public function getPersonal(){
		throw new Exception ('Etherscan.io API doesnt support the personal Module');
	}
	...
	public function getAPIUrl() {
		...
		$api = in_array($this->net,['frontier', 'homestead', 'metropolis','mainnet'])?'api.':							$this->net.'.';
	...
	{
		//die(var_dump($this->balance($address, $block)));
		return $this->balance($address, $block);
		//return \phpEther\Tools\Hex::buffer($bal);
	}
	...
	public function eth_getLogs(\phpEther\Filter $filter ){
		...
		*/
	public function eth_getBlockByNumber($tag = self::TAG_LATEST,$bolean = true ){
		 $params = [
            'module' => "proxy",
